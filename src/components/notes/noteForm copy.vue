...
<template>
    ...
    <div class="toolbar">
      ...
      >
        <button
          ...
          class="flex items-center py-1"
        >
          <PhTable :size="20" class="mr-1" />
          <div
            class="p-1 rounded-full hover:bg-[#ccb478] dark:hover:bg-gray-600 transition-transform duration-200"
            :class="{
              'rotate-180 bg-[#d9c698] dark:bg-gray-600': isTableDropdownOpen,
            }"
          >
            <PhCaretDown :size="16" class="transition-transform duration-300" />
          </div>
  ...
  <div class="fixed-dropdowns">
    ...
    <Transition name="zoom">
      ...
      >
        <div class="py-1" role="menu" @mousedown.stop>
          <li
            class="text-left pt-1 rounded-md flex items-center justify-center"
          >
            <input
              class="max-w-12 p-1 bg-transparent border-b-[1px] border-black dark:border-white outline-none text-center"
              type="text"
              placeholder="Cols"
              v-model="tableCols"
              @click.stop
          ...
          <div
            class="card flex items-center justify-center mx-2 mt-3 my-1 px-2 py-1 cursor-pointer hover:bg-[#d9c698] dark:hover:bg-gray-600 transition-transform duration-200"
            @click="insertTable"
          >
            Insert Table
          </div>
...
<script setup lang="ts">
  ...
  const toggleHeadingDropdown = () => {
    ...
    if (isHeadingDropdownOpen.value) {
  ...
  const tableDropdownStyle = computed(() => {
    if (!tableDropdownRef.value) return {};
    const rect = tableDropdownRef.value.getBoundingClientRect();
    return {
      top: `${rect.bottom + window.scrollY}px`,
      left: `${rect.left + window.scrollX}px`,
      width: '150px',
