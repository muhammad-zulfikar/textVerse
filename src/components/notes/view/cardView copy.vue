...
<template>
  <div class="mx-2 md:mx-4 xl:mx-[4.4rem]" @click="handleOutsideClick">
    ...
    <transition-group
      ...
      tag="ul"
      :class="[
        'relative min-w-[300px] md:mx-auto masonry',
        {
    ...
    >
      <li
        ...
        :key="note.id"
        class="notes bg-cream dark:bg-gray-750 border-[1px] border-black dark:border-gray-400 rounded-lg shadow-lg hover:shadow-2xl transition-all duration-300 break-inside-avoid h-min p-2 cursor-pointer relative group select-none"
        :class="{
...
<script setup lang="ts">
  ...
  onMounted(() => {
    const handleResizeAndCalculate = () => {
      uiStore.handleResize();
      calculateNoteHeights();
    };

    window.addEventListener('resize', handleResizeAndCalculate);
    handleResizeAndCalculate(); // Initial calculation

    // Clean up the event listener when the component is unmounted
    onUnmounted(() => {
      window.removeEventListener('resize', handleResizeAndCalculate);
    });
  });

  const calculateNoteHeights = () => {
    nextTick(() => {
      const notes = document.querySelectorAll('.notes');
      notes.forEach((note) => {
        const noteElement = note as HTMLElement;
        const height = noteElement.getBoundingClientRect().height;
        noteElement.style.gridRowEnd = `span ${Math.ceil(height / 10)}`;
      });
    });
  };
