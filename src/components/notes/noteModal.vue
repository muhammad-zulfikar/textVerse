...
const toolbarProps = computed(() => ({
  ...
  lastEditedDate: editedNote.value.last_edited || editedNote.value.time_created,
  isPinned: editedNote.value.pinned,
  isSaving: isSaving.value,
}));

const hasChanges = computed(() =>
  originalNote.value ? notesStore.hasChanged(originalNote.value, editedNote.value) : false
);
...
async function saveNote() {
  ...
  try {
    ...
    await new Promise((resolve) => setTimeout(resolve, Math.max(0, 500 - (Date.now() - saveStartTime))));
  } catch (error) {
...
async function initializeQuillEditor() {
  if (quillEditorRef.value && !quillEditor.value) {
    ...
    quillEditor.value = new Quill(quillEditorRef.value, {
      theme: 'snow',
      modules: {
        toolbar: !isMobile.value || uiStore.isExpanded
          ? [
    ...
    quillEditor.value.on('text-change', (_delta: any, _oldDelta: any, source: string) => {
