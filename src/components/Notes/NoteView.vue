<template>
    <div v-if="note" class="fixed inset-0 flex items-center justify-center z-50 font-serif" :style="{ backdropFilter: 'blur(2px)' }">
        <div
            ...
            <h1 class="text-xl text-black dark:text-white hover:shadow-xl font-bold mb-4 mt-2">
                <input v-model="note.title"
                    class="w-full p-1 bg-transparent border-0 border-b-2 border-black dark:border-white focus:outline-none" />
            </h1>
            <textarea v-model="note.content"
                class="bg-cream dark:bg-gray-900 hover:shadow-xl text-black dark:text-white w-full p-2 bg-white border-2 border-black dark:border-white rounded focus:outline-none flex-grow resize-none"
                rows="5"></textarea>
