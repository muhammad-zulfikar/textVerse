        ...
        <div :style="{ backgroundColor: note.color }"
            ...
            <div class="flex justify-between mt-2">
                <div class="flex gap-2">
                    ...
                    <button class="flex-shrink-0 w-6 h-6 cursor-pointer" @click="removeSelectedNote">
...
const emit = defineEmits(['close', 'remove', 'save', 'download']);
...
const removeSelectedNote = () => {
    if (props.removeNote && note.value) {
        props.removeNote(note.value.id);
        closeNote();
    }
};
