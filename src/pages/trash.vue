...
<template>
  <div class="trashContainer">
    <div class="mb-[28px] md:mb-[46px] my-6 md:my-8 font-serif" @click="handleOutsideClick">
      <div v-if="!notesStore.deletedNotesLoaded">
        <LoadingSpinner />
      </div>

      <div v-else-if="deletedNotes.length === 0" class="flex flex-col items-center justify-center h-[60vh]">
        <PhTrash :size="100" class="text-gray-400 dark:text-gray-600 mb-4" />
        <p class="text-gray-600 dark:text-gray-400 text-lg font-serif">Trash empty</p>
      </div>
      
      <div v-else class="mx-2 md:mx-4">
        <div class="flex flex-col md:flex-row md:justify-center items-center mb-6 md:mb-8 mx-2">
          <p
          ...
          <Button variant="danger" @click="emptyTrash" class="mt-3 md:mt-0 md:ml-4 text-sm">
        ...
        <transition-group name="list" tag="ul" :class="[
          ...
          <li v-for="note in deletedNotes" :key="note.id"
            ...
            <div>
              <div class="font-serif text-sm mt-2 dark:text-white truncate-text"
              ...
              <div
                ...
                <div class="flex items-center text-left text-[10px] md:text-xs">
                  <div @click.stop="permanentlyDeleteNote(note.id)"
                  ...
                  <div @click.stop="restoreNote(note.id)"
                ...
                <div class="text-left text-[10px] md:text-xs">
                  <p v-if="note.folder !== DEFAULT_FOLDERS.UNCATEGORIZED"
      ...
      <AlertModal :is-open="isDeleteAlertOpen" :message="deleteAlertMessage" @confirm="confirmPermanentlyDeleteNote"
        @cancel="isDeleteAlertOpen = false" />

      <AlertModal :is-open="isEmptyTrashAlertOpen" :message="emptyTrashAlertMessage" @confirm="confirmEmptyTrash"
